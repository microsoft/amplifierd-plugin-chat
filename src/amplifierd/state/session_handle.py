...
class SessionHandle:
    ...
    def _wire_events(self) -> None:
        ...
        for event_name in all_events:
            ...
            try:
                hooks.register(
                    event_name, _on_event, name=f"amplifierd_eventbus_{event_name}"
                )
                registered += 1
            except Exception:
                logger.debug(
                    "Failed to register hook for event %s", event_name, exc_info=True
                )
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Error during session cleanup for %s", self.session_id, exc_info=True
            )
